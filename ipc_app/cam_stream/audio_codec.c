...
int audio_encoder_start(int sampleRate, int *aac_len , int *pcm_len)
{
	...
	/*AAC encoder init */
	if(aac_encoder_init(sampleRate, 16000, &aac_enc_info) != 0)
	{
...
int aac_decoder_init()
{
	...
	{
		DBG( "Unable to set error concealment method\n");
		retval = -12;
		goto err;
	}
	...
	{
		DBG( "Unable to set output channels in the decoder\n");
		retval = -13;
		goto err;
	}
	...
	{
		DBG( "Unable to set DRC boost factor in the decoder\n");
		retval = -14;
		goto err;
	}
	...
	{
		DBG( "Unable to set DRC attenuation factor in the decoder\n");
		retval = -15;
		goto err;
	}
	...
	{
		DBG( "Unable to set DRC reference level in the decoder\n");
		retval = -16;
		goto err;
	}
	...
	{
		DBG( "Unable to set DRC heavy compression in the decoder\n");
		retval = -17;
		goto err;
	}
...
int aac_decode(unsigned char *src,  int  src_len )
{
	...
	if(NULL == out_fp)
	{
		out_fp= fopen("recv.pcm", "wb+");
		if(out_fp == NULL)
		{
			DBG( "open output file %s failed\n","recv.pcm" );
			return -1;
		}
	}
#endif
...
refill:
	...
	if(err != AAC_DEC_OK)
	{
		DBG( "aacDecoder_DecodeFrame failed --- 0x%08x\n", err);
		return -1;
	}
	...
	if(first_decode == 1)
	{
		...
		if(info == NULL)
		{
			DBG( "Unable to get stream info\n");
			return -1;
		}

		if(info->sampleRate <= 0)
		{
			DBG( "Stream info not initialized\n");
			return -1;
		}
...
int aac_decoder_start()
{
	...
	aac_decoder_buf =(unsigned char *)malloc(aac_decoder_len);
	if( !aac_decoder_buf ){
