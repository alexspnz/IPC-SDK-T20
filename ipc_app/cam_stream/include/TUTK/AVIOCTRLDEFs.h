...
typedef enum 
{
    ...
    /*设置设备UTC时间*/
    IOTYPE_USER_IPCAM_SET_TIME_REQ              = 0x606,
    IOTYPE_USER_IPCAM_SET_TIME_RESP             = 0x607,

    /*获取设备机型*/
    IOTYPE_USER_IPCAM_GET_TYPE_REQ              = 0x680,
    IOTYPE_USER_IPCAM_GET_TYPE_RESP             = 0x681,

    /*获取设备信息*/
    IOTYPE_USER_IPCAM_GET_CAMERA_ALL_REQ        = 0x65E,
    IOTYPE_USER_IPCAM_GET_CAMERA_ALL_RESP       = 0x65F,

    /*获取设备所在区域(国内,海外)*/
    IOTYPE_USER_IPCAM_GET_DEVICE_AREA_REQ       = 0x690,
    IOTYPE_USER_IPCAM_GET_DEVICE_AREA_RESP      = 0x691,

    /*设置设备所在区域(国内,海外)*/
    IOTYPE_USER_IPCAM_SET_DEVICE_AREA_REQ       = 0x692,
    IOTYPE_USER_IPCAM_SET_DEVICE_AREA_RESP      = 0x693,

    /*设置设备spk音量*/
    IOTYPE_USER_IPCAM_SET_CAMERA_VOL_REQ        = 0x65C,
    IOTYPE_USER_IPCAM_SET_CAMERA_VOL_RESP       = 0x65D,
...
typedef struct
{
    long utc_time;  // the number of seconds passed
}SMsgAVIoctrlTimeReq;
...
typedef struct
{
    int type;   //0:卡片机 1:摇头机 2:枪机 3:全景壁挂
    int reserved;
}SMsgAVIoctrlDeviceType;
...
typedef struct
{
    int camera;         //IPC开关-> C, 定时使IPC开关-> D.bit0-bit1: 00 CD off, 01 CD on, 10 C off D on, 11 C on D off;
                        //简单点，0，关闭摄像机，定时关机失效；1、3，摄像机开关显示为开，定时关机生效；2 暂未使用 
    int leaveHomeMode;
    int infraredMode;
    int rotateState;
    int cameraMicphone;
    int cameraVoicePrompt;
    int cameraCueLight;
    int cameraVoiceNum;
    int TimeZone;
    int cameraAutoCruise;
    int reserved[10];

}SMsgAVIoctrlGetCameraInfoResp;
...
typedef struct
{
    ...
    int isCN; //0:国外，1:国外 -1:未指向（默认）
}SMsgAVIoctrlDeviceAreaReqResq;
