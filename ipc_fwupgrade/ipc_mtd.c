...
int ipc_mtd_erase(char *devicename)
{
	...
	if(fd <= 0)
	{
		DBG("[%s:%d] failed to open[%s] with: %s\n", __func__, __LINE__, devicename, strerror(errno));
		close(fd);
		return -1;
	}
//    CHECK(fd > 0, -1, "failed to open[%s] with: %s\n", devicename, strerror(errno));
    ...
    CHECK(mtd.erasesize == 32*1024, -1, "Error with: %d\n", mtd.erasesize);
